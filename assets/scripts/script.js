jQuery.noConflict()(function ($) {
    ...
    $(window)
      .on("resize", function () {
        $(".vlt-footer-fixed .vlt-content-holder").css({
          "margin-bottom":
            $('.vlt-footer-holder[data-footer-fixed="1"]').outerHeight() + "px",
        });
      })
      .trigger("resize");
  }
  ...
  function vlthemes_aside_menu() {
    ...
    $(".vlt-navigation-aside li.menu-item-has-children > a").addClass(
      "no-anims"
    );

    $(document).on(
      "click",
      ".vlt-navigation-aside li.menu-item-has-children > a",
      function (e) {
        e.preventDefault();
    ...
    $(document).on(
      "click",
      ".vlt-navigation-aside, .vlt-aside-menu-toggle",
      function (e) {
        e.stopPropagation();
      }
    );
  ...
  function vlthemes_fullscreen_menu() {
    ...
    $(".vlt-navigation-fullscreen li.menu-item-has-children > a").addClass(
      "no-anims"
    );

    $(document).on(
      "click",
      ".vlt-navigation-fullscreen li.menu-item-has-children > a",
      function (e) {
        e.preventDefault();
        $(this).parent("li").toggleClass("sub-menu-opened");
      }
    ...
    $(document).on(
      "click",
      ".vlt-navigation-fullscreen, .vlt-fullscreen-menu-toggle",
      function (e) {
        e.stopPropagation();
      }
    );
  ...
  function vlthemes_preloader() {
    $animsition.animsition({
      ...
      linkElement:
        'a:not([target="_blank"]):not([href^="#"]):not([href^="mailto"]):not(.ajax-btn, #cancel-comment-reply-link, .comment-reply-link, .no-anims, .author-anims-link, .lightbox-link, .vlt-cats-toggle a)',
  ...
  function vlthemes_infinite_scroll() {
    ...
    var maxPages = parseInt(infinity_scroll.maxPages),
      postsContainer =
        ".vlt-postlist-standard, .vlt-portfolio-grid, .vlt-postlist-products",
    ...
    $(postsContainer).infinitescroll(
      {
        loading: {
          finishedMsg:
            '<a href="#" class="infinite-scroll-msg">' + loadMoreNone + "</a>",
          msgText:
            '<a href="#" class="infinite-scroll-msg">' + loadMore + "</a>",
          speed: 0,
        },
  ...
  function vlthemes_progress_bar_shortcode() {
    ...
    element.each(function () {
      $(this).one("inview", function () {
        var $this = $(this),
          percent = parseInt(
            $this.find(".vlt-progress-bar-percent").text(),
            10
          );
